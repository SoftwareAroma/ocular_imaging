...
def train_or_test(options):
    ...
    gen_layers = get_default_gen_layers(input_dim=options.latent_dim, output_dim=128*128*3)
    disc_layers = get_default_disc_layers(input_dim=128*128*3)
    classifier_layers = get_default_classifier_layers(input_dim=128*128*3, num_classes=options.num_classes)
    
    print("Creating netowrk")
    generator = Generator(input_dim=options.latent_dim, layers=gen_layers)
    discriminator = Discriminator(layers=disc_layers)
    classifier = Classifier(num_classes=options.num_classes, layers=classifier_layers)
